...
// fakeKeyStore is an in-memory crypto.KeyStore, standing in for
...
// GetOrCreateWrappedDEK's "first caller wins" contract under -race.
type fakeKeyStore struct {
	mu    sync.Mutex
	rows  map[string][]byte // tenantID -> wrapped DEK; absent means no row
	shred map[string]bool   // tenantID -> shredded
}

func newFakeKeyStore() *fakeKeyStore {
	return &fakeKeyStore{rows: map[string][]byte{}, shred: map[string]bool{}}
}

func (f *fakeKeyStore) GetOrCreateWrappedDEK(_ context.Context, tenantID string, candidate []byte) ([]byte, bool, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.shred[tenantID] {
		return nil, true, nil
	}
	if existing, ok := f.rows[tenantID]; ok {
		return existing, false, nil
	}
	f.rows[tenantID] = candidate
	return candidate, false, nil
}

func (f *fakeKeyStore) GetWrappedDEK(_ context.Context, tenantID string) ([]byte, bool, bool, error) {
	f.mu.Lock()
	defer f.mu.Unlock()
	if f.shred[tenantID] {
		return nil, true, true, nil
	}
	wrapped, ok := f.rows[tenantID]
	if !ok {
		return nil, false, false, nil
	}
	return wrapped, false, true, nil
}
...
func TestCipher_ShreddedTenantEncryptFailsClosed(t *testing.T) {
	store := newFakeKeyStore()
	c := newTestCipher(t, store)
	ctx := context.Background()
	tenant := "tenant-a"
	...
	if _, err := c.Encrypt(ctx, tenant, "new description"); !errors.Is(err, crypto.ErrTenantKeyShredded) {
		t.Errorf("Encrypt for a shredded tenant = %v, want ErrTenantKeyShredded", err)
	}
}
...
// TestCipher_ConcurrentFirstUse proves GetOrCreateWrappedDEK's race is
